#Aim:  study if the number of clutches in a season is influenced by pair duration, experience or age
...
dat_subset<-dat[,c( "N_Clutches_Pair", "breeding_exp_pair_nogaps",
...
mod1 = brms::brm(N_Clutches_Pair ~ breeding_exp_pair_nogaps.z*predation+
...
dat_subset <- within(dat_subset, Count <- ave(N_Clutches_Pair, list(N_Clutches_Pair, breeding_exp_pair_nogaps,predation), FUN=length))  
...
int<-ggplot()+
  ...
  geom_point(aes(size = dat_subset$Count[dat_subset$predation=="natural"],
                 x=sd(dat_subset$breeding_exp_pair_nogaps)*dat_subset$breeding_exp_pair_nogaps.z[dat_subset$predation=="natural"]+
                   mean(dat_subset$breeding_exp_pair_nogaps),y=dat_subset$N_Clutches_Pair[dat_subset$predation=="natural"],
                 color="natural")) +
  ...
  geom_point(aes(size = dat_subset$Count[dat_subset$predation=="protected"],
                 x=(sd(dat_subset$breeding_exp_pair_nogaps)*dat_subset$breeding_exp_pair_nogaps.z[dat_subset$predation=="protected"]+
                      mean(dat_subset$breeding_exp_pair_nogaps))+0.2,y=dat_subset$N_Clutches_Pair[dat_subset$predation=="protected"],
                 color="protected")) +
  
  labs(x="\nPair duration (years)",
       y="Clutches (pair/season)\n",
       title = "",
       size="N Pairs")+
...
tab_model(mod1,file="n_clutches_review2.doc", transform=NULL, show.icc = FALSE, show.zeroinf = FALSE, show.re.var=TRUE)
...
#   2.5%      97.5% 
#   0.04280008 0.18507625 



#Male age 

mean(s$Dad_min_Seasons.z+s$predationprotected.Dad_min_Seasons.z)
# 0.0285918
quantile(s$breeding_exp_pair_nogaps.z+s$breeding_exp_pair_nogaps.z.predationprotected,probs = c(0.025, 0.975))
# 
# 2.5%       97.5% 
...
mod2 = brms::brm(N_Clutches_Pair ~ breeding_exp_pair_nogaps.z*predation+
                   breeding_exp_Mom_nogaps.z*predation+
                   breeding_exp_Dad_nogaps.z*predation+
                   Mom_min_Seasons.z*predation+
                   Dad_min_Seasons.z*predation+
                 ...
                 data = dat_subset2,
                 family = "lognormal",
                 file="N_Clutches_season_trim_rev2")
